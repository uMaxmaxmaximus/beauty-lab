...
module.exports = class Tree
	...
	initDirectives: (node, component, scope)->
		for directive in @directives then do (directive)=>
			dir = directive(node, component, scope)
			node.on 'destroy', => dir.destructor?()
		return
	...
	initClasses: (node, component, scope)->
		for own className, exp of @classes then do (className, exp)=>
			bind = node.bindClass(className, exp, component, scope)
			node.on 'destroy', => bind.destroy()
		return


	initProps: (target, component, scope)->
		for own prop, exp of @props then do (prop, exp)=>
			bind = ui.bind(target, prop, component, exp, scope)
			target.on 'destroy', => bind.destroy()
		return


	initTextExp: (node, component, scope)->
		exp = node.value
		watcher = ui.watch component, exp, (value)=>
			node.value = value
		, scope
		node.on 'destroy', => watcher.destroy()
		return
