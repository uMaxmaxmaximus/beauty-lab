...
createStringExp = (code)->
	...
	for part, index in parts when part
		...
		if index % 2
			...
			compiledExpParts.push "(
				#{VAR_REF} = (#{compiledPart}),
				(#{VAR_REF} == null) || (#{VAR_REF} instanceof Promise)
				? '' : String(#{VAR_REF}))"
		else
			# string part
			part = part.replace /('|\r\n|\r|\n)/img, "\\$1"
			part = "'#{part}'"
			compiledExpParts.push(part)

	result = compiledExpParts.join('+')
...
class TokenList extends Array
	...
	constructor: (@code = null)->
		if TokenList.cache[@code]
			return TokenList.cache[@code]

		@structures = null
		if @code?
			@parseCode(@code)
			TokenList.cache[@code] = @
		return
