...
module.exports = class PropObserver
	...
	constructor: (@context, @prop, @handler, init = off)->
		...
		if @context instanceof Array and @prop is 'length'
			@setArrayLengthObserver()
		else
			if @propCanChange
	...
	setArrayLengthObserver: =>
		oldLength = @context[@prop]
		ArrayObserver = require('./array-observer')
		@arrayLengthObserver = new ArrayObserver @context, =>
			length = @context[@prop]
			if length isnt oldLength
				@handler(length)
				oldLength = length
		return


	destroyArrayLengthObserver: =>
		@arrayLengthObserver.destroy()
		return
